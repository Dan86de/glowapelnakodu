...
export function MuteButton({ player }: { player: PlayerAPI }) {
  return (
    <button
      type="button"
      className="group relative rounded-md hover:bg-zinc-100 focus:outline-none focus:ring-2 focus:ring-zinc-400 focus:ring-offset-2 md:order-none"
      onClick={() => player.toggleMute()}
      aria-label={player.muted ? 'Wyciszony' : 'Wycisz'}
    >
      <div className="absolute -inset-4 md:hidden" />
      <MuteIcon
        muted={player.muted}
        className="h-6 w-6 fill-zinc-500 stroke-zinc-500 group-hover:fill-zinc-700 group-hover:stroke-zinc-700"
      />
    </button>
  )
}
